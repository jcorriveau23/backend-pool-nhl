...
pub async fn respond_trade (
    ...
    match pool::respond_trade(db, &user_id.to_string(), &body.name, body.is_accepted, body.trade_id).await {
        Ok(data) => {
            Ok(Json(data))
        }
        Err(e) => {
...
pub async fn protect_players (
    ...
    match pool::protect_players(db, &user_id.to_string(), &body.name, &body.forw_protected, &body.def_protected, &body.goal_protected, &body.reserv_protected ).await {
        Ok(data) => {
            Ok(Json(data))
        }
        Err(e) => {
