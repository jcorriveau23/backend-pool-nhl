...
pub async fn delete_pool(
    ...
    if pool.is_none() {
        ...
        let delete_result = collection.delete_one(doc!{"name": _pool_name.clone()}, None).await?;
...
pub async fn select_player(
    ...
    if pool_unwrap.context.is_none(){
        return Ok(create_error_response("There is no context to that pool yet.".to_string()).await);
    }
    ...
    if pool_unwrap.participants.is_none(){
        return Ok(create_error_response("There is no participants in that pool yet.".to_string()).await);
    }
    ...
    if !is_added {
        if (pooler_roster.chosen_reservists.len() as u8) < pool_unwrap.number_reservists {
            ...
            return Ok(create_error_response("Not enough space for this player.".to_string()).await);
        }
    }
...
pub async fn create_trade(
    ...
    if !pool_context.pooler_roster.contains_key(&_trade.proposed_by) || !pool_context.pooler_roster.contains_key(&_trade.ask_to) {
        return Ok(create_error_response("The users in the trade are not in the pool.".to_string()).await);
    }
    ...
    for trade in trades.iter() {
        if (matches!(trade.status, TradeStatus::NEW) || matches!(trade.status, TradeStatus::ACCEPTED)) && 
            (trade.proposed_by == *_user_id && trade.ask_to == *_user_id){
                return Ok(create_error_response("User can only have one active trade at a time.".to_string()).await);
            }
    }

    // does the the from or to side has too much items in the trade ?

    if (_trade.from_items.picks.len() + _trade.from_items.players.len()) == 0 || (_trade.to_items.picks.len() + _trade.to_items.players.len()) == 0 {
        return Ok(create_error_response("There is no items traded on one of the 2 sides.".to_string()).await);
    }

    // Maximum of 5 items traded on each side ?

    if (_trade.from_items.picks.len() + _trade.from_items.players.len()) > 5 || (_trade.to_items.picks.len() + _trade.to_items.players.len()) > 5 {
    ...
    if !validate_trade_possession(&_trade.from_items, &pool_context, &_trade.proposed_by).await || 
        ...
        return Ok(create_error_response("One of the to pooler does not poccess the items list provided for the trade.".to_string()).await);
    }
...
pub async fn cancel_trade(
    ...
    if !matches!(trades[_trade_id as usize].status, TradeStatus::NEW) {
        return Ok(create_error_response("The trade is not in a valid state to be cancelled.".to_string()).await);
    }

    // validate only the owner can cancel a trade

    if trades[_trade_id as usize].proposed_by != *_user_id {
        return Ok(create_error_response("Only the one that created the trade can cancel it.".to_string()).await);
    }
...
pub async fn respond_trade(
    ...
    if !matches!(trades[_trade_id as usize].status, TradeStatus::NEW) {
        return Ok(create_error_response("The trade is not in a valid state to be responded.".to_string()).await);
    }

    // validate only the owner can cancel a trade

    if trades[_trade_id as usize].ask_to != *_user_id {
        return Ok(create_error_response("Only the one that was ask for the trade can accept it.".to_string()).await);
    }
    ...
    trades[_trade_id as usize].status = if _is_accepted {TradeStatus::ACCEPTED} else {TradeStatus::REFUSED};
...
pub async fn fill_spot(
    ...
    if !pooler_roster.contains_key(_user_id) {
        return Ok(create_error_response("The pooler is not a participant of the pool.".to_string()).await);
    }
    ...
    !pooler_roster[_user_id].chosen_reservists.contains(_player) {
        return Ok(create_error_response("The player should only be in the reservist pooler's list.".to_string()).await);
    }
...
pub async fn protect_players(
    ...
    let tot_player_protected = _forw_protected.len() + _def_protected.len() + _goal_protected.len() + _reserv_protected.len();

    if tot_player_protected as u8 != pool_unwrap.next_season_number_players_protected {
        return Ok(create_error_response("The number of selected players is not valid".to_string()).await);
    }

    // Validate that the participant realy possess the selected players.

    let mut is_selected_players_valid = false;

    for player in _forw_protected.iter() {
        is_selected_players_valid = validate_player_possession(player, &pooler_context, _user_id ).await;

        if !is_selected_players_valid {
            return Ok(create_error_response("The pooler does not possess one on the selected forwards".to_string()).await);
        }
    }
    ...
    for player in _def_protected.iter() {
        ...
        if !is_selected_players_valid {
            return Ok(create_error_response("The pooler does not possess one on the selected defenders".to_string()).await);
        }
    }
    ...
    for player in _goal_protected.iter() {
        ...
        if !is_selected_players_valid {
            return Ok(create_error_response("The pooler does not possess one on the selected goalies".to_string()).await);
        }
    }
    ...
    for player in _reserv_protected.iter() {
        ...
        if !is_selected_players_valid {
            return Ok(create_error_response("The pooler does not possess one on the selected reservisists".to_string()).await);
        }
    }
    ...
    for participant in pool_unwrap.participants.unwrap().iter() {
        if  (pooler_context.pooler_roster[participant].chosen_forwards.len() + 
            pooler_context.pooler_roster[participant].chosen_defenders.len() + 
            pooler_context.pooler_roster[participant].chosen_forwards.len() +
            pooler_context.pooler_roster[participant].chosen_forwards.len()) as u8 != pool_unwrap.next_season_number_players_protected {
...
async fn validate_trade_possession(_trading_list: &TradeItems, _pool_context: &PoolContext, _participant: &String) -> bool {
...
async fn validate_player_possession(_player: &Player, _pool_context: &PoolContext, _participant: &String) -> bool {
    match _player.position {
        Position::F => {
            if !_pool_context.pooler_roster[_participant].chosen_forwards.contains(_player) && !_pool_context.pooler_roster[_participant].chosen_reservists.contains(_player) {
                return false;
            } 
        }
        Position::D => {
