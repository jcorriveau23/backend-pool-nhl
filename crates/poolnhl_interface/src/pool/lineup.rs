...
//! `score_by_day` did), one event is recorded per change. The lineup effective
//! on any date is the latest event on or before that date; points are then
//! derived from the shared `day_leaders`.
...
/// (one document per change). Persisted in the `pool_lineups` collection.
...
/// The lineup effective on `date` for a single participant: the latest event on
/// or before that date. Returns empty slices if the participant has no event
/// yet (e.g. a date before the draft).
pub fn lineup_as_of<'a>(
    ...
    events
        .iter()
        .filter(|event| event.effective_date.as_str() <= date)
...
mod tests {
    ...
    fn extract_collapses_unchanged_days_into_one_event() {
        ...
        let mut events = extract_lineup_events("p", &score_by_day);
    ...
    fn lineup_as_of_returns_the_latest_prior_event() {
        let events = extract_lineup_events(
            "p",
            &HashMap::from([
                (
        ...
        assert_eq!(
            lineup_as_of(&events, "2025-09-30"),
            (&[][..], &[][..], &[][..])
        );
        // Between the two events: the first lineup still holds.
        assert_eq!(lineup_as_of(&events, "2025-10-03").0, &[10, 11]);
        // On/after the change: the new lineup.
        assert_eq!(lineup_as_of(&events, "2025-10-10").0, &[10, 12]);
    }
}
